...
function dijkstra(diagonal,sourceArray,destinationArray){
	...
	dist[mapping1D(sourceArray[0], sourceArray[1], maxCols)] = 0;
	...
	let idx = mapping1D(sourceArray[0], sourceArray[1], maxCols);
	pq.enqueue(idx,0);
    ...
    while(!pq.isEmpty()){
        ...
        if(removeCell[0] == destinationArray[0] && removeCell[1] == destinationArray[1]){
            pathfound=true;
            break;
        }
...
class PNode { 
    ...
    { 
        ...
        this.index = index; 
        //"distance" stores the distance of each node from the sourceArray node (start cell)
...
class PriorityQueue { 
    ...
    // a node for priority queue that contains index and distance from sourceArray
